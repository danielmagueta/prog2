...
public class TestFractions {
  ...
  public static void main(String[] args) {
    ...
    assert ((x.add(y)).add(z)).equals(x.add(z.add(y))) : "(x+y)+z = x+(y+z)";
    
    //Comutativa da multiplicação
    assert (x.multiply(y)).equals(y.multiply(x)) : "(x*y) = (y*x)";
    
    //Associatividade da multiplicação
    assert ((x.multiply(y)).multiply(z)).equals(x.multiply(y.multiply(z))) : "(x*y)*z = x*(y*z)";
    
    //Distributividade da multiplicação
    assert (x.multiply(y).add(x.multiply(z))).equals(x.multiply(y.add(z))) : "(x*y)+(x*z) = x*(y+z)";
