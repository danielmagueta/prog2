...
public class SocialNetwork {
  ...
  public SocialNetwork(int maxSize, int sizeExpansion) {
    ...
    this.members = new Person[maxSize];
    this.requests = new FriendshipRequest[maxSize];
  ...
  private void extendRequests() {
	    FriendshipRequest[] newfr = new FriendshipRequest[requests.length+sizeExpansion];
	    System.arraycopy(requests,0,newfr,0,numRequests);
	    requests = newfr;
  }
  ...
  public void addFriendshipRequest(String m1, String m2) {
	 int im1 = memberIndex(m1); 
	 int im2 = memberIndex(m2);
	 if  (requests.length == numRequests) {
    	extendRequests();
    }
    FriendshipRequest fr = new FriendshipRequest(im1,im2);
    requests[numRequests] = fr;
    numRequests++;
  }
  ...
  public int numPendingRequests(String memberName) {
	int cont = 0;
	int memberindex = memberIndex(memberName);
	for(int i = 0; i < numRequests(); i++) {
    	if(requests[i].requester() == memberindex && requests[i].pending()) {cont++;}}
    return cont;
  }
  ...
  public String oldestFriend(String memberName) {
	  int oldestbirth = Integer.MAX_VALUE;
	  String oldest = null;
	  int memberindex = memberIndex(memberName);
	  for (int i = 0; i < numRequests(); i++) {
		  if (requests[i].accepted()&&memberindex ==requests[i].requested()) {
			  if (members[requests[i].requester()].birthYear()< oldestbirth){
				  oldestbirth = members[requests[i].requester()].birthYear();
				  oldest = members[requests[i].requester()].name();
			  }
		  }
		  if (requests[i].accepted()&&memberindex ==requests[i].requester()) {
			  if (members[requests[i].requested()].birthYear()< oldestbirth){
				  oldestbirth = members[requests[i].requested()].birthYear();
				  oldest = members[requests[i].requested()].name();
			  }
		  } 
	  }
	  return oldest;
